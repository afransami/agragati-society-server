...
async function run() {
  try {
    ...
    // DB_USER=agragati-society
    // DB_PASS=ycMoBCbwIUceGFQt
...
// save users email and role in db
    ...
    // upload bus details 
      app.post('/uploadBusDetails', async (req, res) => {
        const bus = req.body
      ...
      app.get('/search', async (req, res) => {
        const { from, to, startDate } = req.body;
      
        try {
          // Perform the MongoDB query
          const searchResults = await busCollection.find({
...
app.get ('/', (req, res)=>{
    res.send ('Agragati Society is on')
})

app.listen(port, ()=>{
    console.log(`Agragati Society is on port ${port}`);
})
